      ...
      <Container>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col sm:flex-row items-start sm:items-end justify-between mb-12">
          <div>
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-display font-bold tracking-tight">Our Recent Work</h2>
          ...
          {projects.map((p, i) => {
            ...
            return (
              <Link key={p.slug} href={`/portfolio/${p.slug}`}>
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="group rounded-2xl overflow-hidden border border-border/50 bg-card transition-all duration-300 hover:shadow-lg">
