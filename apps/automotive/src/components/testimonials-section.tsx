...
export function TestimonialsSection({ testimonials = [] }: { testimonials: Array<{ id: string; client_name: string; client_title: string | null; content: string }> }) {
  return (
    <Section variant="gold">
      <Container>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <Badge variant="gold" className="mb-4">Testimonials</Badge>
          <AnimatedText text="Trusted by Leaders" as="h2" className="text-3xl sm:text-4xl lg:text-5xl font-display font-bold tracking-tight" />
        </motion.div>
